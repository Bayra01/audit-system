...
function BarChart({ data }) {
  ...
  useEffect(() => {
    ...
    types.forEach((t, i) => {
      ...
      let yOff = 0;
      sevs.forEach(sev => {
        const c = data.filter(x => x.type === t && x.severity === sev).length;
        if (!c) return;
        const h = (c / maxVal) * cH;
      ...
      const lbl = t.length > 9 ? t.slice(0,9)+"…" : t;
      ctx.fillText(lbl, pad.l + i*gap + gap/2, H - 8);
    });
    ctx.strokeStyle = "#e8e8e8"; ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(pad.l, pad.t); ctx.lineTo(pad.l, pad.t + cH); ctx.stroke();

    const lbls = [["Маш өндөр","#ef5350"],["Өндөр","#ff7043"],["Дунд","#ffa000"],["Бага","#43a047"]];
    lbls.forEach(([l,c],i) => {
      ctx.fillStyle = c; ctx.fillRect(pad.l + i*85, 2, 10, 8);
      ctx.fillStyle="#555"; ctx.font="10px Arial"; ctx.textAlign="left";
      ctx.fillText(l, pad.l + i*85 + 13, 10);
    });
  }, [data]);
...
function DoughnutChart({ data }) {
  ...
  useEffect(() => {
    ...
    const total = nw + pr + dn || 1;
    const vals = [nw, pr, dn];
    const clrs = ["#1e88e5","#ffa000","#43a047"];
    const lbls = ["Шинэ","Явцтай","Дууссан"];
    ...
    ctx.font = "bold 24px Arial"; ctx.fillStyle="#1a2d45"; ctx.textAlign="center";
    ctx.fillText(total, cx, cy+8);
    ctx.font = "11px Arial"; ctx.fillStyle="#999"; ctx.fillText("Нийт", cx, cy+24);
    lbls.forEach((l,i) => {
      const lx = 8 + i*(W/3);
      ctx.fillStyle=clrs[i]; ctx.beginPath(); ctx.arc(lx+6, H-12, 5, 0, 2*Math.PI); ctx.fill();
      ctx.fillStyle="#555"; ctx.font="11px Arial"; ctx.textAlign="left";
      ctx.fillText(`${l} (${vals[i]})`, lx+14, H-8);
    });
  }, [data]);
  return <canvas ref={ref} style={{width:"100%",height:"230px",display:"block"}} />;
}
...
function LineChart({ data }) {
  ...
  useEffect(() => {
    ...
    const H = 190;
    const ctx = canvas.getContext("2d");
    const dn = cntArr(data,"status","done");
    const total = data.length;
    const months = ["1-р","2-р","3-р","4-р","5-р","6-р"];
    const ds = [
      { d:[8,12,7,15,10,total], clr:"#1e88e5", fill:"rgba(30,136,229,.1)", lbl:"Нийт" },
      { d:[3, 7,4, 9, 6,dn],    clr:"#43a047", fill:"rgba(67,160,71,.08)", lbl:"Шийдсэн" },
    ];
    const pad = { t:24, r:16, b:36, l:30 };
    const cW = W-pad.l-pad.r, cH = H-pad.t-pad.b;
    const maxV = Math.max(...ds.flatMap(x=>x.d), 1);
    ctx.clearRect(0,0,W,H);
    ...
    ds.forEach(({ d, clr, fill, lbl }) => {
      ...
      ctx.lineTo(pts[0][0], pad.t+cH); ctx.closePath();
    ...
    ds.forEach(({clr,lbl},i)=>{
      ctx.fillStyle=clr; ctx.fillRect(pad.l+i*80, 4, 12, 8);
      ctx.fillStyle="#555"; ctx.font="10px Arial"; ctx.textAlign="left";
      ctx.fillText(lbl, pad.l+i*80+15, 12);
    });
  }, [data]);
  return <canvas ref={ref} style={{width:"100%",height:"190px",display:"block"}} />;
}
      ...
      <div className="modal">
        ...
        <div className="fg">
          <div className="fl fg-full">
            ...
            <input value={form.name||""} onChange={e=>set("name",e.target.value)} placeholder="Зөрчлийн нэр…" />
          </div>
          <div className="fl fg-full">
            <label>Зөрчлийн төрөл</label>
            <input value={form.type||""} onChange={e=>set("type",e.target.value)} placeholder="Журам зөрчих, Хугацаа хэтрэх…" />
          </div>
          <div className="fl">
            <label>Эрсдэлийн зэрэг</label>
            <select value={form.severity||"low"} onChange={e=>set("severity",e.target.value)}>
...
export default function Dashboard({ user, onLogout }) {
  ...
  const filtered = data.filter(x=>
    ...
    (!fSev  || x.severity===fSev) &&
    (!fStat || x.status===fStat)
  );
  const pages = Math.ceil(filtered.length/PER)||1;
  const slice = filtered.slice((pg-1)*PER, pg*PER);

  function openAdd() {
    setModal({ open:true, title:"Зөрчил нэмэх", initial:{name:"",type:"",severity:"low",date:today(),status:"new"}, editId:null });
  }
  function openEdit(id) {
    const row = data.find(x=>x.id===id);
    setModal({ open:true, title:"Зөрчил засварлах", initial:{...row}, editId:id });
  }
  function saveV(form) {
    if (!form.name?.trim()||!form.type?.trim()||!form.date) { showToast("Бүх талбарыг бөглөнө үү","err"); return; }
    if (modal.editId) {
      setData(d=>d.map(x=>x.id===modal.editId?{...x,...form}:x));
      showToast("Амжилттай засварлагдлаа ✅");
    } else {
      setData(d=>[...d,{id:Date.now(),...form}]);
      showToast("Амжилттай нэмэгдлээ ✅");
    }
  ...
  function emailTemplate() {
    setEmail({
      to: email.to,
      subj:"Зөрчлийн мэдэгдэл — Хяналтын Систем",
  ...
  return (
    <div style={{display:"flex",height:"100vh",width:"100vw",overflow:"hidden",fontFamily:"Arial, Helvetica, sans-serif",position:"fixed",top:0,left:0}}>
            ...
            <div>
              ...
              <div className="stats">
                <div className="stat s-cyan">
                  ...
                  <div className="stat-l">Нийт зөрчил</div>
                </div>
                <div className="stat s-red">
                  <div className="stat-n">{cntArr(data,"severity","critical")}</div>
                  <div className="stat-l">Маш өндөр эрсдэл</div>
                </div>
                <div className="stat s-orange">
                  <div className="stat-n">{cntArr(data,"severity","high")}</div>
                  <div className="stat-l">Өндөр эрсдэл</div>
                </div>
                <div className="stat s-amber">
                  <div className="stat-n">{cntArr(data,"status","progress")}</div>
                  <div className="stat-l">Явцтай</div>
                </div>
                <div className="stat s-green">
                  <div className="stat-n">{cntArr(data,"status","done")}</div>
                  <div className="stat-l">Дууссан</div>
                </div>
              </div>

              <div className="charts-row">
                <div className="chart-card">
                  <div className="chart-title">Төрлөөр ангилсан зөрчил</div>
                  <BarChart data={data} />
                </div>
                <div className="chart-card">
                  <div className="chart-title">Төлвийн харьцаа</div>
                  <DoughnutChart data={data} />
                </div>
                <div className="chart-card">
                  <div className="chart-title">Сарын хандлага</div>
                  <LineChart data={data} />
                </div>
              </div>
          ...
          {tab==="violations" && (
            <div>
              ...
              <div className="toolbar">
                <input className="t-search" placeholder="🔍 Нэр, төрлөөр хайх…"
              ...
              <div className="tbl-wrap">
                <table>
                  <thead>
                    <tr>
                      <th>#</th><th>Нэр / Хариуцагч</th><th>Төрөл</th>
                      <th>Эрсдэл</th><th>Огноо</th><th>Төлөв</th><th>Үйлдэл</th>
                    </tr>
                  </thead>
                  <tbody>
                    {!slice.length
                        ...
                        <tr key={row.id}>
                          ...
                          <td>
                            <div className="row-acts">
                              ...
                              {user?.role==="Admin" &&
                                <button className="btn b-red btn-sm" onClick={()=>delV(row.id)}>🗑</button>}
                            </div>
                          </td>
                        </tr>
                      ))
            ...
            <div>
              ...
              <div className="email-wrap">
                <div className="email-card">
                  ...
                  <div className="fl" style={{marginBottom:13}}>
                    <label>Хүлээн авагч (To)</label>
                    <input type="email" placeholder="example@company.mn"
                      value={email.to} onChange={e=>setEmail(em=>({...em,to:e.target.value}))} />
                  </div>
                  <div className="fl" style={{marginBottom:13}}>
                    <label>Гарчиг (Subject)</label>
                    <input type="text" placeholder="Зөрчлийн мэдэгдэл"
                      value={email.subj} onChange={e=>setEmail(em=>({...em,subj:e.target.value}))} />
                  </div>
                  <div className="fl" style={{marginBottom:20}}>
                    <label>Агуулга</label>
                    <textarea placeholder="Имэйлийн агуулга…"
      ...
      {/* ══ TOAST ══ */}
      {toast.msg && (
        <div className={`toast show t-${toast.type}`}>{toast.msg}</div>
      )}
    </div>
  );
}